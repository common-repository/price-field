...
function ic_block_price_enqueue() {
	...
	$localize      = array(
		'strings' => array(
			...
			'options'            => __( 'Options', 'price-field' ),
			'disable_add_cart'   => __( 'Disable add to cart button', 'price-field' ),
			'search_product'     => __( 'Search Product', 'price-field' ),
			'select_product'     => __( 'Select Product', 'price-field' ),
			'search_placeholder' => __( 'Search by item name and select it in the section below', 'price-field' ),
			'edit_block'         => __( 'Edit selected product.', 'price-field' ),
		)
	);
	if ( has_action( 'price_table', 'ic_cart_add_button' ) ) {
		$localize['price_add_cart_added'] = 1;
