...
class product_price_filter extends WP_Widget {
	...
	function __construct() {
		if ( is_plural_form_active() ) {
			...
			$label    = sprintf( __( '%s Price Filter', 'price-field' ), ic_ucfirst( $names['singular'] ) );
			$sublabel = sprintf( __( 'Filter %s by price.', 'price-field' ), ic_lcfirst( $names['plural'] ) );
		} else {
			$label    = __( 'Catalog Price Filter', 'price-field' );
			$sublabel = __( 'Filter items by price.', 'price-field' );
		}
		$widget_ops = array(
		...
		if ( get_integration_type() != 'simple' ) {
			if ( ic_if_show_filter_widget( $instance, get_class( $this ) ) ) {
                ...
                <div class="price-filter ic_ajax <?php design_schemes( 'box' ) ?> <?php echo $class ?>"
                     ...
                     data-ic_responsive_label="<?php _e( 'Price', 'price-field' ) ?>"
                     data-ic_ajax_data="<?php echo esc_attr( json_encode( array(
					     'instance' => $instance,
					     'args'     => $args
				     ) ) ) ?>">
                    <span class="filter-label"><?php _e( 'Price', 'price-field' ) ?>:</span>
	...
	function form( $instance ) {
		if ( get_integration_type() != 'simple' ) {
			...
			?>
            <p>
                <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'price-field' ); ?>
            ...
            <p><input class="widefat" id="<?php echo $this->get_field_id( 'shortcode_support' ); ?>"
                    ...
                    for="<?php echo $this->get_field_id( 'shortcode_support' ); ?>"><?php _e( 'Enable also for shortcodes', 'price-field' ); ?></label>
            </p><?php
			do_action( 'ic_price_filter_widget_settings', $instance, $this );
		} else {
			//implecode_warning( sprintf( __( '%s is disabled due to a lack of main catalog listing.%s', 'price-field' ), __( 'Price filter widget', 'price-field' ), ic_catalog_notices::create_listing_page_button() ) );
